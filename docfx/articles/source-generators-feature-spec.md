...
11. `GeneratedPivotChartPage` — implemented with a strict attributed-only schema, globally ordered row/column/filter/value fields, a generated and callback-configurable `PivotTableModel`, a pivot-derived chart, a bounded range chart using cached numeric selectors, bounded long-form Region/value series, incremental stable-key mapping, bidirectional grid/chart selection, ReactiveUI source-generated state and commands, reactive period updates, metric and series-source switching, passive compiled XAML, ViewModel/runtime/generator tests, and deterministic Avalonia Headless screenshot coverage.
