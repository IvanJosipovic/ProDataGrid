...
| F18 analytics | Implemented | Typed pivot and outline fields, globally ordered axis/value factories, generated configurable pivot/outline model construction, direct numeric chart selectors, compile-time formula dependency and syntax validation, runtime indexed formula definitions, relative-reference fill, bounded spreadsheet range projection, bounded long-form series grouping, incremental stable-key maps, and bidirectional chart/grid selection synchronization are available. |
| F19 generated views | Implemented | Avalonia and ReactiveUI code-only views, compiled binding indexers, custom bases, recipes, named slots, automation metadata, dynamic resource theme keys, validated class tokens, compiled diagnostics-status projection, state bridges, typed loading/empty/error projections, hierarchical and formula-model bindings, retry-command bindings, typed routed-event command bridges, typed ReactiveUI interaction responses, activation/DataContext-scoped subscriptions, protected handler factories, and `[DataGridViewRegistration]` mappings for existing XAML views are available. |
...
- Its collection views are created by generated schema factories. `ApplyCollectionViewSorting` compiles descriptor lists against generated accessors and installs comparer-based sort descriptions; generated group fields replace property-path grouping.
...
Implemented DynamicData API: named controllers own one `SourceList<T>` or keyed `SourceCache<T,TKey>` connection, typed revision-aware sort/filter/search subjects, replace-aware binding, optional final scheduler boundary, observable errors/completion, and deterministic disposal. `PipelineTransformMethod` adds a compile-time validated private/static/instance hook whose parameter and return type must be the exact `IObservable<IChangeSet<TItem>>` or keyed `IObservable<IChangeSet<TItem,TKey>>` shape. It executes directly after `Connect()` and before generated operations, allowing user-owned transforms, group/summary side projections, telemetry, or test-scheduler instrumentation without reflection; `PDGSG004` rejects mismatches.
...
Implemented transfer API: generated schemas expose typed clipboard, fill, clipboard-import-model, and fill-model factories. The DataGrid adapters resolve stable `ColumnKey` values to generated edit fields, apply rectangular or one-value multi-cell paste, extrapolate numeric/date/time/duration sequences, translate relative formulas, or perform cyclic copy, enforce hard cell and payload limits, publish structured keyed errors, and commit each operation as one undo batch. `IFormulaFillTranslator` is capability-injected; `ExcelFormulaFillTranslator` shifts mixed absolute/relative A1 references over the existing AST while preserving R1C1 and structured references. `FormulaFillTranslatorType` emits a direct configured factory after `PDGSG137` validation. Generated views compile-time validate and bind the two model interfaces and configure selection, edit triggers, copy mode, read-only state, and conservative add/delete defaults.
...
Implemented conditional-formatting API: repeatable `[DataGridConditionalFormat]` metadata emits cached `DataGridGeneratedConditionalRule<TItem,TValue>` instances behind `IDataGridGeneratedConditionalRule`. Built-in comparisons, inclusive `Between` ranges, null checks, and allocation-free `Contains`/`StartsWith`/`EndsWith` text predicates use the canonical typed getter and compile-time-validated operands. Text rules default to ordinal comparison and can select any `StringComparison`; custom rules call a validated static `bool (TItem, TValue)` method. Invalid typed operands and incompatible text targets report `PDGSG009`. Stable rule and column IDs, priority, `StopIfTrue`, theme key, and `ConditionalFormattingTarget.Cell` or `Row` are preserved.

Every schema exposes the typed `ConditionalRules` collection and `CreateConditionalFormattingModel()`. The runtime factory creates custom-predicate descriptors with `ConditionalFormattingValueSource.Item`, so evaluation calls the cached generated predicate and never resolves a property path. Generated views accept `ConditionalFormattingModelPropertyName`, validate the member against `IConditionalFormattingModel`, and bind it directly to `DataGrid.ConditionalFormattingModelProperty`; `PDGSG131` rejects missing or incompatible members. The formal sample validates seven cell rules and two row rules, including range, case-insensitive text, custom cross-field predicates, priorities, reactive updates, rule clearing/restoration, and rendered theme resources through Avalonia Headless.
...
- Static formula syntax validation through the source-generator analyzer package.
...
Implemented pivot/chart API: `[DataGridPivotAxis]`, `[DataGridPivotValue]`, `[DataGridChartField]`, `[DataGridOutlineField]`, and `[DataGridFormulaField]` contribute roles to the canonical `AnalyticsFields` manifest. Generated `CreatePivotAxisFields` and `CreatePivotValueFields` methods globally order matches by declared order and stable column key. Pivot values support validated calculated `Formula`/`Dependencies`, direct `IPivotAggregator` factories, and exact per-axis/per-value configure hooks. Schema and assembly/namespace policies may declare exact `PivotConfigureMethod` and `OutlineConfigureMethod` hooks. `CreatePivotTableModel(IEnumerable, Action<PivotTableModel>?)` installs typed row, column, filter, and value selectors with refresh suspended, invokes generated schema configuration followed by the application callback, and only then activates the model. Configuration failures dispose the partial model before propagating the exception.

`DataGridGeneratedOutlineAdapter` converts globally ordered `OutlineGroup` and `OutlineDetail` metadata into direct-selector `OutlineGroupField` and aggregated `OutlineValueField` instances, including exact field hooks and direct custom aggregator factories. Generated providers expose field factories and `CreateOutlineReportModel(IEnumerable, Action<OutlineReportModel>?)`; generated schema configuration runs before the optional callback. Advanced behavior is additive through `IDataGridGeneratedAdvancedAnalyticsField` and `DataGridGeneratedAdvancedAnalyticsOptions`; the base analytics interface and original constructor signatures remain unchanged for custom and binary consumers. The semantic `SchemaHash` now includes group, summary, band, analytics, formula dependency, factory, and configuration-hook contracts. Numeric chart roles additionally expose a cached selector through the additive `IDataGridGeneratedNumericAnalyticsField` contract. The optional `ProDataGrid.Charting` adapter uses this delegate directly for value, X, and size dimensions, avoiding the boxed compatibility getter and conversion path for generated numeric properties. `DataGridGeneratedChartRangeProjection` selects series by stable generated column key, applies a bounded exact row window, and preserves customized series when only row coordinates change. `DataGridGeneratedLongFormChartDataSource` partitions by the generated series discriminator, aligns category/value pairs, aggregates duplicates, observes changes, and bounds rows and emitted series. `DataGridGeneratedListChartKeyMap<TItem,TKey>` incrementally maintains generated identity across observable list changes; `DataGridGeneratedChartSelectionSynchronizer<TItem,TKey>` connects it to generated selection with origin-based feedback suppression and replaceable index mappings. Formula columns are validated by `PDGSG138` using the shared production tokenizer and allocation-light `ExcelFormulaSyntaxValidator`; the same validator is a public runtime/editor API and has a dedicated benchmark. The formal `GeneratedPivotChartPage` adds declarative layout/field hooks and a calculated margin measure. The formal `GeneratedOutlineDragDropPage` adds declarative outline configuration and a direct custom range aggregator.
...
The formal `GeneratedReactiveViewRecipesPage` applies four `GenerateDataGridView` declarations to one ReactiveUI ViewModel. `GridOnly`, `Explorer`, `Spreadsheet`, and `Analytics` receive independent generated classes, stable `GeneratedRecipe` constants, automation IDs, read-only/editable configuration, and named toolbar/recipe slots while sharing one strict attributed-only schema and one collection view. The Analytics declaration additionally applies dynamically resolved view/grid resource keys, direct class tokens for the view/grid/toolbar/content slot, and a compiled diagnostics-status binding. Its Explorer and Analytics search boxes use the same compiled two-way query binding and generated canonical-field predicates. ViewModel, generator, Avalonia Headless, and deterministic screenshot tests cover the contract without view-owned event handlers. `PDGSG139` rejects invalid presentation tokens and incompatible diagnostics properties for direct, assembly, and namespace policies.
...
Implemented collection-view lifetime API: `DefaultPageSize`, `InitialPageIndex`, `InitialCurrency`, `PreserveCurrentItemByKey`, and `PreserveSelectionByKey` flow through direct, assembly, and namespace schema policies and participate in the semantic schema hash. `CreateCollectionView` applies grouping, paging, and currency directly. Keyed schemas emit `CreateCollectionViewController`, which composes the generated key accessor, keyed selection controller, Avalonia identity selection model, and active `DataGridCollectionView`. Its detached snapshot and `Refresh`, `SetPageSize`, `ReplaceView`, and `TryMoveCurrentToKey` operations preserve currency and selection across unloaded pages and replacement instances. The controller keeps a sticky key when the current item is off page and uses the DataGrid's unpaged selection source rather than expanding page changes into selection resets. `PDGSG140` rejects invalid defaults. The formal ReactiveUI `PagingSelectionPage` replaces runtime auto-columns with generated definitions and validates paging, refresh, page-size, and source-replacement behavior through runtime, generator, ViewModel, and Avalonia Headless tests.
...
- Lightweight global schema composition followed by independently cached per-schema semantic discovery and emission.
...
Current status: implemented. Direct schema ownership/provider collision resolution is separated from per-schema semantic discovery and emission. Focused incremental tests cover unrelated-schema reuse plus invalidation from property changes, owner options, and provider collisions. On the documented .NET 10 Arm64 local reference, a one-schema edit in a 32-schema compilation used 51% of cold-generation time and 33% of its managed allocation. The generated-only NativeAOT application additionally verifies schema, controller, registry, Avalonia view, ReactiveUI view, custom-base, trimming-analysis, native-link, and native-execution compatibility in CI.
...
Current status: the formal grouping/summaries, conditional-formatting, and header-filter samples are complete. They cover rendered generated summary descriptions, incremental Add/Remove/Replace/Reset aggregates, typed comparison/custom formatting predicates, cached reflection-free runtime descriptors, all typed filter-editor profiles, bounded local/remote distinct values, stale suppression, live command availability, cell/row targets, generated model binding, reactive updates, runtime rule toggling, and view-scoped grid operations. Banding, chooser/layout, templates, row details, and custom drawing already have generated APIs and focused tests.
...
Current status: the Avalonia and ReactiveUI strategies, all seven layout recipes, multiple views per ViewModel, custom bases and protected customization hooks, compiled binding indexers, activation-scoped adapters, routed-event command/interaction bridges, view-state projections, reflection-free registrations, DI helpers, localization, accessibility, and diagnostics metadata are implemented. The formal four-recipe ReactiveUI page validates distinct layouts over one generated ViewModel and schema. The formal custom-implementations page validates factory/hook/policy composition with an application base and derived generated view. The formal assembly/namespace-policy page validates defaults, explicit precedence, registry lookups, nested exclusion, and registered XAML view creation. Future MVVM frameworks can be added through the existing strategy boundary without changing schema discovery.

### Phase 7 — analytics integrations

1. Add pivot and outline metadata generation.
2. Add formula analyzer/value resolver generation.
...
Current status: pivot, chart, outline, formula, and drag/drop integrations are implemented with formal samples. The pivot/chart sample covers ordered row/column/filter/value fields, generated pivot-model construction and customization, pivot-derived series, bounded spreadsheet range projection, bounded long-form series grouping, direct generated numeric chart selectors, incremental stable-key mapping, bidirectional grid/chart selection, reactive source changes, and deterministic headless rendering. The indexed spreadsheet sample covers generated formula definitions, model binding, relative fill, and compile-time syntax validation through the shared production tokenizer. The outline/drag-drop sample covers generated group/value factories, configurable report construction, direct selectors, stable-key Move/Copy requests, domain validation, rejection status, and reactive report refresh. Capability-gated analytics benchmarks cover range construction, long-form and outline projection, stable-key lookup, and allocation-light formula syntax validation. Phase 7 exit criteria are satisfied.
...
Current status: implemented and continuously validated. ProDiagnostics exposes six generated item schemas covering all seven grids plus eight generated XAML view mappings; ProDiagnostics.Viewer adds two streaming schemas. Collection-view creation, sorting, and grouping use generated typed operations. The validation lane covers streaming and hierarchical paths, multi-schema ViewModels, strict compiled bindings, registry/schema tests, and Avalonia Headless view creation. Repository source audits report no inline DataGrid columns and no disabled compiled-binding scopes in either project.
...
3. `GeneratedDynamicDataSourceCachePage` — implemented with a generated keyed `SourceCache` pipeline, a typed user transform/telemetry stage, replace-aware upstream sorting, filtering and search, a generated identity selection model and ReactiveUI view binding, deterministic cache batches, error counters, idempotent disposal, and runtime plus Avalonia Headless proof that a selected replacement instance keeps the same stable key after moving rows.
...
11. `GeneratedPivotChartPage` — implemented with a strict attributed-only schema, globally ordered row/column/filter/value fields, validated declarative schema/field hooks, a calculated margin measure with stable dependencies, an optional caller-configurable `PivotTableModel`, a pivot-derived chart, a bounded range chart using cached numeric selectors, bounded long-form Region/value series, incremental stable-key mapping, bidirectional grid/chart selection, ReactiveUI source-generated state and commands, reactive period updates, metric and series-source switching, passive compiled XAML, ViewModel/runtime/generator tests, and deterministic Avalonia Headless screenshot coverage. `GeneratedOutlineDragDropPage` additionally validates declarative outline configuration and a direct custom range aggregator.
...
13. `GeneratedCustomImplementationsPage` — implemented with a user-owned compiled bound-column factory, schema-level and per-column configure hooks, domain severity comparer, generated edit-controller validator, direct weighted-score summary calculator, custom ReactiveUI base, derived protected view hooks, compiled toolbar command/status bindings, ViewModel/generator tests, and deterministic Avalonia Headless screenshot coverage.
14. `GeneratedAssemblyNamespacePolicyPage` — implemented with non-recursive model, ViewModel, and ReactiveUI view namespace policies; strict streaming/high-frequency defaults; an explicit attributed-only schema, permissive ViewModel, and spreadsheet-view override; generated registry lookup by item type and schema ID; nested-namespace exclusion; reflection-free XAML shell registration; generator/ViewModel/Avalonia Headless tests; and deterministic screenshot coverage.
15. `GeneratedHeaderFiltersPage` — implemented with all seven standard editor profiles, a real generated distinct-value header flyout, bounded local and remote values, cancellation-resistant stale-response suppression, cached per-field header commands, external command-state invalidation, typed filtering/search, live visibility, and generated ReactiveUI interaction handling for pin/freeze/autosize/reset, with runtime/generator/ViewModel/Avalonia Headless tests and deterministic screenshot coverage.
