...
`RowDragUpdated` is the live row-drag session event. It exposes the fully recomputed `DataGridRowDragSession`, including pointer position, modifiers, hovered row, drop zone, and the current requested/effective drag effect. `RowDragCompleted` represents committed drops only; canceled drags raise `RowDragCanceled`.
