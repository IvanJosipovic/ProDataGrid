...
The virtual surface supports text, masked text, numeric, checkbox, date, time, image,
progress, and hierarchical columns when they have compatible typed accessors. Text,
masked-text binding formatting, numeric, date, and time formatting, text column typography, image stretch, progress styling,
...
| Typed text, masked text, numeric, checkbox, date, time, image, progress, hierarchy display | Drawn by `Virtualized` | Uses one surface and zero display-cell controls. |
