...
`Create{Name}RemoteQueryController`, `Initialize{Name}RemoteQuery`, `Query{Name}Async`, `Prefetch{Name}Async`, `{Name}RemoteQuery`, and `Dispose{Name}RemoteQuery` are generated for a keyed remote controller. A newer foreground request cancels the previous foreground request and advances the revision. A provider response is accepted only when both its revision and the controller's current revision match; canceled or stale results return `null` and cannot overwrite current rows. `StateChanged` reports loading, errors, and stale suppression. Offset and opaque-cursor paging are both supported through `DataGridPageRequest`.

`Prefetch{Name}Async` is an explicit cache-only policy hook. It reserves the same monotonic revision sequence but neither cancels nor supersedes foreground work and does not change loading/error/content state. It requires a stable cache key, propagates provider failures to its caller, is canceled when the controller is disposed, and returns `false` when caching is disabled or disposal wins. A later foreground query for the same key wraps the cached page with its current foreground revision. The generated remote sample prefetches the next offset page after each accepted page; cursor providers can use the accepted `NextCursor` in the same API.
...
The supported flags are `SelectionChanged`, `SelectionChanging`, `CurrentCellChanged`, `Sorting`, `BeginningEdit`, `CellEditEnding`, `CellEditEnded`, `RowEditEnding`, `RowEditEnded`, `CellPrepared`, `CellClearing`, and `CellValueChanged`; `Editing`, `CellLifecycle`, and `All` are convenience combinations. `DataGridGeneratedViewEvent<TItem>` exposes typed row/current/proposed items, stable column keys, row index, edit action, selection origin and guarantee, zero-copy typed selection projections, proposal rows/cells/columns, hierarchy node/path, realized cell/row containers, and old/new committed values. A command can synchronously set `Cancel` for edit or transactional selection events and `Handled` for routed triggers; generated code copies feedback back before the DataGrid continues.

`SelectionChanging` is the pre-commit contract: setting `Cancel` rejects an `AtomicPreflight` proposal before selection, current cell, anchor, currency, focus, or scrolling state changes. `CellPrepared` and `CellClearing` allow bounded container-scoped services without code-behind, while `CellValueChanged` reports only successful DataGrid editor commits. These are synchronous event contracts; long-running work should be queued by the ViewModel instead of blocking the UI thread.
