            ...
            try
            {
                ...
                switch (e.Action)
                {
                    case NotifyCollectionChangedAction.Add:
                        ...
                        if (ShouldAutoGenerateColumns && !_owner.HasAutoGeneratedColumns())
                        {
                            // The first item of an untyped source can reveal the schema, so initialize
                            // everything only when there are no generated columns to preserve.
                            _onCollectionAddRemoveNewRowPlaceholder = false;
                            _placeholderRowIndexDuringAdd = null;
